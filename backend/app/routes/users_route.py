...
def create_user():
    ...
    if not name or not email or not password:
        return jsonify({"error": "Es necesario tener todos los datos"}), 400

    if User.query.filter_by(email=email).first():
        return jsonify({"error": "Ya existe ese correo"}), 400

    password_hash = generate_password_hash(password)
    new_user = User(name=name, email=email, password=password_hash, rol=rol)
    db.session.add(new_user)
    # Registra un cambio en nuestra db como tipo github
    db.session.commit()
    return jsonify({"msg": "Usuario creado con exito", "user": new_user.to_dict()}), 201
...
def change_status(user_id):
    ...
    if user:
        return jsonify({"msg": "Estatus actulaizado", "activo": user.state}), 200
    return jsonify({"msg": "Algo salio mal al intentaractualiar el estado"}), 400
