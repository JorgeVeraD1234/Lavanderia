...
def search_client_by_name(name):
    return Client.query.filter(Client.name.ilike(f'%{name}%')) #LIKE
...
def update_client(client_id, updated_data):
    ...
    for key, value in updated_data.items():
        setattr(client, key, value) ## client que bvas a cambiar, la key es el atributo y value el nuevo valor
    db.session.commit()
    return client
